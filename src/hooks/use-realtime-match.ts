  ...
  useEffect(() => {
    ...
    const connectToRealtime = () => {
      try {
        ...
        // Retry with fast delays in development
        const isDev = process.env.NODE_ENV === 'development'
        const maxRetries = isDev ? 1 : 3
        
        if (retryCount < maxRetries) {
          const delay = isDev ? 200 : 1000 * Math.pow(2, retryCount)
          ...
          console.log("⚡ Fast-failing in development mode - not retrying further")
        }
      } finally {
