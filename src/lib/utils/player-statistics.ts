...
export function calculateDetailedStats(matches: Match[], playerId: string): DetailedStats {
  ...
  const matchDurations = playerMatches
    .map(m => {
      ...
      const opponent = m.playerOne?.$id === playerId ? m.playerTwo?.firstName : m.playerOne?.firstName
  ...
  playerMatches.forEach(match => {
    const isPlayer1 = match.playerOne?.$id === playerId
    ...
    // TODO: Update this when pointLog structure is properly typed
    // Currently pointLog is string[], not an array of point objects
  ...
  playerMatches.forEach(match => {
    const opponentId = match.playerOne?.$id === playerId ? match.playerTwo?.$id : match.playerOne?.$id
    const opponentName = match.playerOne?.$id === playerId 
      ? `${match.playerTwo?.firstName} ${match.playerTwo?.lastName}`.trim()
      : `${match.playerOne?.firstName} ${match.playerOne?.lastName}`.trim()
  ...
  const comebackWins = playerMatches.filter(match => {
    if (match.winnerId !== playerId) return false
    const isPlayer1 = match.playerOne?.$id === playerId
    const score = match.score
  ...
  const decidingSetWins = decidingSetMatches.filter(m => m.winnerId === playerId).length
  ...
  const recentWinRate = recentMatches.length > 0
    ? (recentMatches.filter(m => m.winnerId === playerId).length / recentMatches.length) * 100
    : 0
  
  const olderWinRate = olderMatches.length > 0
    ? (olderMatches.filter(m => m.winnerId === playerId).length / olderMatches.length) * 100
    : 0
