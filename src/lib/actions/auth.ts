...
import { preloadUserData } from "../utils/batch-operations"
...
export async function signUp(formData: FormData) {
  ...
  try {
    ...
    // Step 4: Warm cache with user data for better performance
    try {
      await preloadUserData(user.$id)
    } catch (error) {
      // Don't fail registration if cache warming fails
      console.warn("Cache warming failed during signup:", error)
    }
...
export async function signIn(formData: FormData) {
  ...
  try {
    ...
    try {
      await preloadUserData(session.userId)
    } catch (error) {
      // Don't fail login if cache warming fails
      console.warn("Cache warming failed during signin:", error)
    }
