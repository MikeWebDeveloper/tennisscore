...
import { User, Plus, Star, Check, Users } from "lucide-react"
...
export function CompactPlayersStep({
  ...
  const createPlayerOptions = (excludeIds: string[] = []): ComboboxOption[] => {
    ...
    // Add load all players option if not all are loaded
    if (!allPlayersLoaded && totalPlayerCount && totalPlayerCount > players.length && onLoadAllPlayers) {
      options.push({
        value: "load-all",
        label: `Load All Players (${totalPlayerCount - players.length} more)`,
        group: t('actions'),
  ...
  const handlePlayerChange = async (player: "one" | "two" | "three" | "four", value: string) => {
    if (value === "create-new") {
      setShowCreatePlayerDialog(true)
    } else if (value === "load-all" && onLoadAllPlayers) {
      await onLoadAllPlayers()
    } else {
