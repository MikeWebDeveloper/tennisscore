...
export function MatchDetails({ match }: MatchDetailsProps) {
  ...
  const formatDuration = (durationMinutes?: number) => {
    ...
    if (durationMinutes === undefined || durationMinutes === null) return t("unknown")
  ...
  const formatFullDate = (dateString: string) => {
    const localeCode = locale === 'cs' ? 'cs-CZ' : 'en-GB'
            ...
            <div className="text-center space-y-3">
              ...
              {match.status === "In Progress" && (
                <Button size="sm" asChild className="mt-3">
                  <Link href={`/matches/live/${match.$id}`} prefetch={true}>
                          ...
                          return (
                            <div key={index} className="flex items-center justify-between p-3 rounded-lg bg-muted/30">
                              <div className="flex items-center gap-3">
                                <span className="text-sm font-medium">{t('setLabel')} {index + 1}</span>
                                {setWinner && (
              ...
              return (
                <Card>
                  <CardContent className="p-8 text-center">
                    ...
                    <p className="text-muted-foreground mb-2">{t('noDetailedPointLog')}</p>
                    <p className="text-sm text-muted-foreground">{t('enableDetailedLogging')}</p>
                  </CardContent>
                </Card>
              )
            }
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {breakPointConversion > 40 
                            ? t('excellentConversionRate')
                            : breakPointConversion > 25
                            ? t('goodConversionRate')
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {breaksOfServe === 0
                            ...
                            ? t('strongServiceGames')
                            : t('serviceGamesUnderPressure')}
                        </div>
                      </div>
                    </CardContent>
                  </Card>
                  ...
                  <CardContent>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      ...
                      <div className="text-center p-4 rounded-lg bg-yellow-50">
                        ...
                        <div className="text-xs text-yellow-600">{t('doubleFaults')} - {getTeamName("team1")}</div>
                      </div>
                      <div className="text-center p-4 rounded-lg bg-orange-50">
                        <div className="text-2xl font-bold text-orange-900">{stats.doubleFaultsByPlayer[1]}</div>
                        <div className="text-xs text-orange-600">{t('doubleFaults')} - {getTeamName("team2")}</div>
                      </div>
                    </div>
                    ...
                    <div className="text-sm text-muted-foreground">
                      <p className="mb-2"><strong>{t('performanceInsightsColon')}</strong></p>
                      <ul className="space-y-1 text-xs">
                        <li>• {t('servicePower')}: {stats.acesByPlayer[0] + stats.acesByPlayer[1]} {t('acesServedTotalLabel')}</li>
                        <li>• {t('pressurePoints')}: {breakPointsCreated} {t('breakPointOpportunities')}</li>
                        <li>• {t('matchRhythm')}: {totalPoints} {t('pointsOver')} {setsPlayed} {t('sets')} ({(totalPoints / Math.max(setsPlayed, 1)).toFixed(1)} {t('pointsPerSet')})</li>
                        {match.startTime && match.endTime && (
                          <li>• {t('playingPace')}: {(() => {
                            const durationMs = Date.parse(match.endTime) - Date.parse(match.startTime)
                            const durationMinutes = durationMs / (1000 * 60)
                            return (totalPoints / durationMinutes).toFixed(1)
                          })()} {t('pointsPerMinute')}</li>
                        )}
                      </ul>
                    </div>
                  </CardContent>
