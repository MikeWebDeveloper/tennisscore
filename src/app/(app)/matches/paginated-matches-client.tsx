...
import { Plus, ArrowUpDown, ArrowUp01, ArrowDown10, Loader2, Filter } from "lucide-react"
...
function enhanceMatches(matches: Match[], players: Player[]): EnhancedMatch[] {
...
export function PaginatedMatchesClient({ 
  ...
  const handleDateFilterChange = async (newFilter: DateFilter) => {
    ...
    try {
      ...
      const enhancedMatches = enhanceMatches(result.matches, players)
  ...
  const loadMoreMatches = async () => {
    ...
    try {
      ...
      const enhancedNewMatches = enhanceMatches(result.matches, players)
