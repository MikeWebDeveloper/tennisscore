...
export function AdminMatchesClient({ 
  ...
  return (
    <div className="p-4 max-w-6xl mx-auto">
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold mb-2">{t('allMatchesAdminView')}</h1>
        <p className="text-muted-foreground text-sm">
          {t('viewAllMatchesCreated')}
        </p>
      </div>
      ...
      <div className="mb-6">
        <div className="relative max-w-md">
          ...
          <Input
            placeholder={t('searchByPlayerNames')}
            value={searchTerm}
      ...
      <div className="mb-4 text-sm text-muted-foreground">
        {isLoading ? (
          ...
          <span>
            {t('showingMatches').replace('{count}', matches.length.toString()).replace('{total}', total.toString())}
        ...
        <div className="mt-6 text-center">
          ...
          >
            {isLoading ? (
              ...
              t('loadMoreMatches').replace('{remaining}', (total - matches.length).toString())
            )}
          </Button>
        </div>
      )}
