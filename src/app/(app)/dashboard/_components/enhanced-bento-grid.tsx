...
import { useFramerCardAnimation } from "@/hooks/use-framer-card-animation"
...
export function EnhancedBentoGrid({ matches, mainPlayer, isLimitedData = false, totalMatches }: EnhancedBentoGridProps) {
  ...
  const { calculateStats, isWorkerSupported, isWorkerReady } = useStatsWorker()
  
  // Calculate comprehensive stats (fallback)
  const fallbackStats = calculateEnhancedStats(matches, mainPlayer?.$id)
  
  // Use worker for complex calculations when available
  useEffect(() => {
    if (isWorkerSupported && isWorkerReady && mainPlayer && matches.length > 0) {
      setIsCalculating(true)
      calculateStats(matches, mainPlayer.$id)
        .then(stats => {
  ...
  }, [isWorkerSupported, isWorkerReady, mainPlayer, matches, calculateStats])
  
  // Use worker stats if available, otherwise fallback
  const stats = workerStats || fallbackStats
  ...
  const cardAnimation = useFramerCardAnimation({
  ...
  return (
    ...
    >
      ...
      <div data-dashboard-container className="space-y-6 md:space-y-8">
        ...
        {isLimitedData && totalMatches && totalMatches > matches.length && (
          <motion.div 
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm font-medium text-blue-900 dark:text-blue-100">
                    Showing recent {matches.length} of {totalMatches} matches
                  </p>
                  <p className="text-xs text-blue-700 dark:text-blue-300 mt-1">
                    For detailed analysis of all matches, visit the Player Stats page
                  </p>
                </div>
              </div>
              <Button variant="outline" size="sm" asChild className="text-blue-600 dark:text-blue-400 border-blue-300 dark:border-blue-700">
                <Link href="/player-statistics">View Full Stats</Link>
              </Button>
            </div>
          </motion.div>
        )}
